...
\usepackage[a4paper,bindingoffset=0.2in,%
            left=0.7 in,right= 0.7 in,top= 0.7 in,bottom=0.7in,%
...
\author{Julian Bautista*, Alex Pavlakis*, Advait Rajagopal\footnote{The New School for Social Research}}
%\date{\today}		
...
The Bayesian approach combines data and prior information, which coupled with the correct model yield more precise inferences than either source on its own.
\fi
Model development involves setting up a joint probability distribution that accounts for all observed data and unobserved parameters. The model should include all knowledge of the experiment or data collection process and should be logically consistent with scientific nature of the problem at hand. We approach model development in three steps.
 ...
 \end{enumerate}
  To make this concrete, let $\theta$ be a parameter of interest and $y$ be data from which we want to estimate $\theta$.  Then the prior distribution is represented as $p(\theta)$ and the likelihood is $p(y | \theta)$.
 Bayesian inference involves multiplying these distributions together according to Bayes' rule to estimate a \emph{posterior distribution} of parameters conditional on data. The product of the prior and likelihood, the posterior distribution is denoted by $ p(\theta | y)$.
 \begin{align*}
 p(\theta | y) \propto p(\theta) p(y |\theta)
 \end{align*}
The Bayesian approach combines data and prior information in a way that can yield more precise inferences that either on its own.
