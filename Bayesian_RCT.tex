...
Figure Y displays the distribution of histograms in each condition in each week.  The distributions appear to condense around zero for both conditions over time, and the distributions in the CBT-GSH condition appear to have longer tails than those in the CBB-GSH+Noom condition.  While the distribution
...
\begin{tabular}{r l}
  ...
  Literature & Effect size is close to zero \\
    \\
  					  & Individual effects come from a common distribution \\
  Exploratory Data Analysis & Time effects come from a common distribution \\
                                            & Treatment effects come from a common distribution \\
   \\
    \hline
\end{tabular}
...
$\alpha$ is an individual-specific intercept, $\beta$ is a time-specific intercept, $\gamma$ is a time-specific treatment effect, $T$ is a treatment indicator, $X$ is a matrix of individual level covariates (age, sex, race, etc), and $\theta$ is a vector of effects.
\\
